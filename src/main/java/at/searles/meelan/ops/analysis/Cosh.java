...
public class Cosh extends SystemInstruction implements DerivableInstruction {

    private static Cosh singleton = null;

    public static Cosh get() {
        if (singleton == null) {
            singleton = new Cosh();
        }
    ...
    protected Const evaluate(FunctionType functionType, List<Tree> args) {
        switch(functionType.returnType()) {
            case real:
                return new Real(Math.cosh(((Real) args.get(0)).value()));
            case cplx:
                return new CplxVal(new Cplx().cosh(((CplxVal) args.get(0)).value()));
            default:
    ...
    public Tree derive(SourceInfo sourceInfo, Var var, List<Tree> args, List<Tree> dargs) {
        ...
        return Mul.get().apply(
                sourceInfo, Arrays.asList(
                        Sinh.get().apply(sourceInfo, Collections.singletonList(x)),
                        dx
                )
        );
    }
