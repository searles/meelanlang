...
public class Tanh extends SystemInstruction implements DerivableInstruction {

    private static Tanh singleton = null;

    public static Tanh get() {
        if (singleton == null) {
            singleton = new Tanh();
        }
    ...
    protected Const evaluate(FunctionType functionType, List<Tree> args) {
        switch(functionType.returnType()) {
            case real:
                return new Real(Math.tanh(((Real) args.get(0)).value()));
            case cplx:
                return new CplxVal(new Cplx().tanh(((CplxVal) args.get(0)).value()));
            default:
    ...
    public Tree derive(SourceInfo sourceInfo, Var var, List<Tree> args, List<Tree> dargs) {
        ...
        return Mul.get().apply(
                sourceInfo, Arrays.asList(
                        Sub.get().apply(
                                sourceInfo, Arrays.asList(new Int(1),
                                        Sqr.get().apply(
                                                sourceInfo, Collections.singletonList(
                                                        Tanh.get().apply(sourceInfo, Collections.singletonList(x))
                                                )
                                        ))
