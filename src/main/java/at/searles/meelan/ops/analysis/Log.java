...
public class Log extends SystemInstruction implements DerivableInstruction {
    ...
    protected Const evaluate(FunctionType functionType, List<Tree> args) {
        switch(functionType.returnType()) {
            case real:
                return new Real(Math.log(((Real) args.get(0)).value()));
            case cplx:
                return new CplxVal(new Cplx().log(((CplxVal) args.get(0)).value()));
            default:
    ...
    public Tree derive(SourceInfo sourceInfo, Var var, List<Tree> args, List<Tree> dargs) {
        Tree x = args.get(0);
        Tree dx = dargs.get(0);
        return Div.get().apply(sourceInfo, Arrays.asList(dx, x));
    }
}
