...
public class Sinh extends SystemInstruction implements DerivableInstruction {

    private static Sinh singleton = null;

    public static Sinh get() {
        if (singleton == null) {
            singleton = new Sinh();
        }
    ...
    protected Const evaluate(FunctionType functionType, List<Tree> args) {
        switch(functionType.returnType()) {
            case real:
                return new Real(Math.sinh(((Real) args.get(0)).value()));
            case cplx:
                return new CplxVal(new Cplx().sinh(((CplxVal) args.get(0)).value()));
            default:
    ...
    public Tree derive(SourceInfo sourceInfo, Var var, List<Tree> args, List<Tree> dargs) {
        ...
        return Mul.get().apply(
                sourceInfo, Arrays.asList(
                        Cosh.get().apply(sourceInfo, Collections.singletonList(x)),
                        dx
                )
        );
    }
